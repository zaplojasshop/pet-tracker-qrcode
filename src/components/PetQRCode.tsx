...
export const PetQRCode = ({ petInfo, onReset }: PetQRCodeProps) => {
  ...
  const downloadQRCode = async (format: string) => {
    ...
    canvas.width = 1200; // Alta resolução
    canvas.height = 1200;
    ...
    img.onload = async () => {
      ...
      switch (format) {
        ...
        case "dxf":
          const d = new Drawing();
          // Criar uma representação simplificada do QR code em DXF
          // Adicionar um retângulo base
          d.addLayer('QRCode', Drawing.ACI.WHITE, 'CONTINUOUS');
          d.setActiveLayer('QRCode');
          
          // Obter os dados da imagem do canvas para criar o DXF
          const imageData = ctx.getImageData(0, 0, canvas.width, canvas.height);
          const pixelSize = 0.1; // Tamanho de cada pixel no DXF
          
          for (let y = 0; y < canvas.height; y += 10) {
            for (let x = 0; x < canvas.width; x += 10) {
              const i = (y * canvas.width + x) * 4;
              if (imageData.data[i] < 128) { // Se o pixel for escuro
